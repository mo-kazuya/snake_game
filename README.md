...
- **対戦Transformer 攻撃型 / 防御型 / バランス型** … 同じ3性格を、**対戦フル学習版をベースに
  “報酬”で**作ったモデルです（`train_transformer_battle.py --style`）。模倣で性格を付ける方式は
  ベースが教師（探索AI由来）を追い越した時点で破綻し、実測でも「攻撃型」が撃破率90%→23%と
  **ベースより非攻撃的**になってしまいました。そこで性格を報酬として定義しています:
  攻撃型＝撃破報酬をエサ30個分に、防御型＝生存報酬0.15/tickでスコアには無関心、
  バランス型＝対戦報酬そのもの。各2MステップPPO。
  - 実測（vs探索AI・12×12・20戦）で**攻撃度が距離に単調に出ます**: 相手との平均距離
    **攻撃3.62 / バランス4.11 / 防御5.11**（ベースは4.03）。エサは旧トリオの**3.4〜4.3倍**。
  - モデル同士では**攻撃 vs 防御 = 30%対70%**。密着して競り合う攻撃型は自分も倒されやすく
    （被撃破70%）、強い相手には裏目に出ます。旧BC版の攻撃型には**90%対10%**で圧勝します。
