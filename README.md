...
- **ego/Transformer 対戦特化** … 上のTransformerと**同じアーキテクチャ**を、**2匹対戦環境
  ...
  モードでもそのまま動作します（相手セルが空になるだけ）。同梱モデルは
  **対戦BC（60万遷移・検証精度90.6%）＋対戦PPO 8Mステップ**のフル学習版です
  （以前はCPU環境の制約でBCのみの軽量版でした）。直接対戦60ゲーム（両スポーン側で公平化）で
  **旧軽量版に勝率73%対23%**、**ベース単独版にも53%対42%**。探索AI相手の平均エサも
  **4.55 → 29.97（6.6倍）**に伸び、「競り合いで生き延びて食べ続ける」挙動になりました。
