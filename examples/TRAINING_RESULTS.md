...
LoRAは「ベース単独版の能力を温存したまま小さな対戦デルタだけを学習する」規律ある手法として
機能し、**ベースを明確に上回ります**。一方でモデルが0.88Mと小さくメモリ制約が無い本ケースでは、
LoRAの省メモリ利点は活きず、強さでもフルBCに劣後します。そのため**同梱の `rl_trf_battle` は
最強のフルBC版を維持**し、LoRAは本スクリプトで再現可能な代替レシピとして提供します。より強い
LoRAを狙うなら、注意機構を（`out_proj` はカスタム関数、qkv は `in_proj_weight` を分解して）
LoRA化するか、rank/エポックを増やす、あるいはBCの後にGPUでPPOを重ねる余地があります。

```bash
# 例: 対戦デモを使って LoRA マージ済みドロップインを作る（別ファイルに出力して比較）
python examples/train_transformer_lora.py --dataset /path/battle_demos.npz \
    --r 16 --alpha 32 --epochs 3 --out examples/ppo_snake_transformer_battle_lora.zip
```
